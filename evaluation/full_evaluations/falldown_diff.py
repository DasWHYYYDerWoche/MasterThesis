...
gyros = ["x_gyro", "y_gyro", "z_gyro"]
sensors = ["lShoulderPitch","rShoulderPitch","lElbowYaw","rElbowYaw","lKneePitch","rKneePitch"] + gyros
action = "standup_front"
# kick right
sensor_gaps = {"combined" : []}
total_gap_base = gap_handler_base.get_final_FINAL_gap_avg(lambda x: SimulationGapData.get_total_gap_avg(x, JOINT_NAMES), [action])
total_gap_new = gap_handler_new.get_final_FINAL_gap_avg(lambda x: SimulationGapData.get_total_gap_avg(x, JOINT_NAMES), [action])
joint_gaps = {"combined" : total_gap_new - total_gap_base}
for joint in JOINT_NAMES+["x_gyro", "y_gyro", "z_gyro"]:
    gap_base = gap_handler_base.get_final_FINAL_gap_avg(lambda x: SimulationGapData.get_total_gap_avg(x, [joint]), [action])
    gap_new = gap_handler_new.get_final_FINAL_gap_avg(lambda x: SimulationGapData.get_total_gap_avg(x, [joint]), [action])
...
for gap_data_base, gap_data_new in zip(gap_handler_base.get_all(action), gap_handler_new.get_all(action)):
    sensor_gap_base = gap_data_base.get_total_gap_for_sensors(JOINT_NAMES +["x_gyro", "y_gyro", "z_gyro"])
    sensor_gap_new = gap_data_new.get_total_gap_for_sensors(JOINT_NAMES + ["x_gyro", "y_gyro", "z_gyro"])
    for key,base in sensor_gap_base.items():
        if key not in sensor_gaps.keys():
            sensor_gaps[key] = []
        diff = sensor_gap_new[key] - base
        sensor_gaps[key].append(diff)
        if key in ["x_gyro", "y_gyro", "z_gyro"]:
            continue
        sensor_gaps["combined"].append(diff)
...
plt.ylim(bottom=y_min*(1+y_margin), top=y_max*(1+y_margin))
...
plt.xticks()
plt.savefig(folder + "/" + "standup_front_diff.pdf", bbox_inches='tight')
...
table2.to_csv(folder +"/" + "standup_front_diff.csv", index=False)
...
plt.xticks()
plt.savefig(folder + "/" + "standup_back_diff.pdf", bbox_inches='tight')
...
table2.to_csv(folder +"/" + "standup_back_diff.csv", index=False)