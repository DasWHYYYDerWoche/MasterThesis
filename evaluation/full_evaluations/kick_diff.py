...
total_gap_base = gap_handler_base.get_final_FINAL_gap_avg(lambda x: SimulationGapData.get_total_gap_avg(x, JOINT_NAMES + ["y_gyro"]), [action])
total_gap_new = gap_handler_new.get_final_FINAL_gap_avg(lambda x: SimulationGapData.get_total_gap_avg(x, JOINT_NAMES + ["y_gyro"]), [action])
...
for gap_data_base, gap_data_new in zip(gap_handler_base.get_all(action), gap_handler_new.get_all(action)):
    sensor_gap_base = gap_data_base.get_total_gap_for_sensors(JOINT_NAMES + ["y_gyro"])
    sensor_gap_new = gap_data_new.get_total_gap_for_sensors(JOINT_NAMES + ["y_gyro"])
...
plt.xticks()
plt.savefig(folder + "/" + "kick_right_diff.pdf", bbox_inches='tight')
...
table2.to_csv(folder +"/" + "kick_right_diff.csv", index=False)
...
plt.legend(loc="lower left")
plt.xticks()
plt.savefig(folder + "/" + "kick_left_diff.pdf", bbox_inches='tight')
...
table2.to_csv(folder +"/" + "kick_left_diff.csv", index=False)