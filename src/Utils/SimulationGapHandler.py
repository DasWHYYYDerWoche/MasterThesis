...
class SimulationGapHandler:
    ...
    # -------- averages over all experiments of with the same name --------

    def get_frame_avg(self, action_name : str, hinge_names: Optional[list[str]] = None, start_frame: int = 0, end_frame: int = -1):
        if not action_name in self._sim_gap_data.keys():
            return []
        frame_avg_s = []
        for gap_object in self._sim_gap_data[action_name]:
            gap_object.load()
            frame_avg_s.append(gap_object.get_pos_gap_frame_avg(hinge_names, start_frame, end_frame))
            gap_object.unload()
        div = len(frame_avg_s)
        ret = {}
        for frame_avg in frame_avg_s:
            for key in frame_avg.keys():
                ret[key] += frame_avg[key]
        return {key : (value / div) for key, value in ret.items()}

    def get_joint_avg(self, action_name : str, hinge_names: Optional[list[str]] = None, start_frame: int = 0, end_frame: int = -1):
        if not action_name in self._sim_gap_data.keys():
            return []
        joint_avg_s = []
        for gap_object in self._sim_gap_data[action_name]:
            gap_object.load()
            joint_avg_s.append(gap_object.get_pos_gap_joint_avg(hinge_names, start_frame, end_frame))
            gap_object.unload()
        ret = [0 for _ in range(max([len(joint_avg) for joint_avg in joint_avg_s]))]
        div_at = [0 for _ in range(max([len(joint_avg) for joint_avg in joint_avg_s]))]
        for i, joint_avg in enumerate(joint_avg_s):
            for avg_at_frame in joint_avg:
                ret[i] += avg_at_frame
                div_at[i] += 1
        return [val / div for val, div in zip(ret, div_at)]
