...
class SimulationGapData:
    ...
    def get_pos_gaps(self,
            ...
            -> dict[str, list[float]]:
        """
        Returns the squared difference between the position of the extraction and replay for the given sensors and frames
        """
        return self._calculate_gap(self.get_pos_extraction(sensor_names, start_index, end_index),
                                   self.get_pos_replay(sensor_names, start_index, end_index), self._pos_gap_factor)
    ...
    def get_vel_gaps(self,
            ...
            -> dict[str, list[float]]:
        """
        Returns the squared difference between the velocity of the extraction and replay for the given sensors and frames
        """
        return  self._calculate_gap(self.get_vel_extraction(sensor_names, start_index, end_index),
                                   self.get_vel_replay(sensor_names, start_index, end_index), self._vel_gap_factor)
    ...
    def get_acc_gaps(self,
            ...
            -> dict[str, list[float]]:
        """
        Returns the squared difference between the acceleration of the extraction and replay for the given sensors and frames
        """
        return self._calculate_gap(self.get_acc_extraction(sensor_names, start_index, end_index),
                                  self.get_acc_replay(sensor_names, start_index, end_index), self._acc_gap_factor)
    ...
    def get_total_gaps(self,
            ...
            -> dict[str, list[float]]:
        """
        Returns the average of the pos, vel and acc gaps.
        """
        d_p = self.get_pos_gaps(sensor_names, start_index, end_index)
        d_v = self.get_vel_gaps(sensor_names, start_index, end_index)
    ...
    def get_pos_gap_for_sensors(self,
                                sensor_names: Optional[list[str]] = None,
                                start_index: int = 0,
                                end_index: int = -1)\
            -> dict[str, float]:
        """
        Returns the average position sim gap for the given sensors by averaging over all given frames.
        """
        return {joint_name : float(np.nanmean(sim_gaps)) for joint_name,sim_gaps in
                self.get_pos_gaps(sensor_names, start_index, end_index).items()}

    def get_vel_gap_for_sensors(self,
                                sensor_names: Optional[list[str]] = None,
                                start_index: int = 0,
                                end_index: int = -1) \
            -> dict[str, float]:
        """
        Returns the average velocity sim gap for the given sensors by averaging over all given frames.
        """
        return {joint_name : float(np.nanmean(sim_gaps)) for joint_name,sim_gaps in
                self.get_vel_gaps(sensor_names, start_index, end_index).items()}

    def get_acc_gap_for_sensors(self,
                                sensor_names: Optional[list[str]] = None,
                                start_index: int = 0,
                                end_index: int = -1) \
            -> dict[str, float]:
        """
        Returns the average acceleration sim gap for the given sensors by averaging over all given frames.
        """
        return {joint_name : float(np.nanmean(sim_gaps)) for joint_name, sim_gaps in
                self.get_acc_gaps(sensor_names, start_index, end_index).items()}

    def get_total_gap_for_sensors(self,
                                  sensor_names: Optional[list[str]] = None,
                                  start_index: int = 0,
                                  end_index: int = -1) \
            -> dict[str, float]:
        """
        Returns the average of the pos, vel and acc for_sensors gaps.
        """
        return {joint_name : float(np.nanmean(sim_gaps)) for joint_name,sim_gaps in
                self.get_total_gaps(sensor_names, start_index, end_index).items()}
    ...
    def get_pos_gap_for_frames(self,
            ...
            -> list[float]:
        """
        Returns the average position sim gap in the given frames by averaging over all given sensors.
        """
        return self._get_weighted_sensor_average(self.get_pos_gaps(sensor_names, start_index, end_index))
    ...
    def get_vel_gap_for_frames(self,
            ...
            -> list[float]:
        """
        Returns the average velocity sim gap in the given frames by averaging over all given sensors.
        """
        return self._get_weighted_sensor_average(self.get_vel_gaps(sensor_names, start_index, end_index))
    ...
    def get_acc_gap_for_frames(self,
            ...
            -> list[float]:
        """
        Returns the average acceleration sim gap in the given frames by averaging over all given sensors.
        """
        return self._get_weighted_sensor_average(self.get_acc_gaps(sensor_names, start_index, end_index))
    ...
    def get_total_gap_for_frames(self,
            ...
            -> list[float]:
        """
        Returns the average of the pos, vel and acc for_frames gaps.
        """
        return self._get_weighted_sensor_average(self.get_total_gaps(sensor_names, start_index, end_index))

    # -------- average over both joints and time --------

    def get_pos_gap_avg(self, sensor_names: Optional[list[str]] = None, start_index: int = 0, end_index: int = -1) -> float:
        """
        Returns the position sim gap. Calculated by averaging over all given frames and sensors
        """
        return float(np.nanmean(self.get_pos_gap_for_frames(sensor_names, start_index, end_index)))

    def get_vel_gap_avg(self, sensor_names: Optional[list[str]] = None, start_index: int = 0, end_index: int = -1) -> float:
        """
        Returns the velocity sim gap. Calculated by averaging over all given frames and sensors
        """
        return float(np.nanmean(self.get_vel_gap_for_frames(sensor_names, start_index, end_index)))

    def get_acc_gap_avg(self, sensor_names: Optional[list[str]] = None, start_index: int = 0, end_index: int = -1) -> float:
        """
        Returns the acceleration sim gap. Calculated by averaging over all given frames and sensors
        """
        return float(np.nanmean(self.get_acc_gap_for_frames(sensor_names, start_index, end_index)))

    def get_total_gap_avg(self, sensor_names: Optional[list[str]] = None, start_index: int = 0, end_index: int = -1) -> float:
        """
        Returns the average of the pos, vel and acc avg gaps.
        This is the final gap of a single replay, representing it as a single value.
        """
        return float(np.nanmean(self.get_total_gap_for_frames(sensor_names, start_index, end_index)))

    # testing methods (these should yield the same result (apart from small rounding errors) as the ones above)

    def _get_pos_gap_avg_test(self, sensor_names: Optional[list[str]] = None, start_index: int = 0, end_index: int = -1) -> float:
        return self._get_weighted_sensor_average_single(self.get_pos_gap_for_sensors(sensor_names, start_index, end_index))

    def _get_vel_gap_avg_test(self, sensor_names: Optional[list[str]] = None, start_index: int = 0, end_index: int = -1) -> float:
        return self._get_weighted_sensor_average_single(self.get_vel_gap_for_sensors(sensor_names, start_index, end_index))

    def _get_acc_gap_avg_test(self, sensor_names: Optional[list[str]] = None, start_index: int = 0, end_index: int = -1) -> float:
        return self._get_weighted_sensor_average_single(self.get_acc_gap_for_sensors(sensor_names, start_index, end_index))

    def _get_total_gap_avg_test(self, sensor_names: Optional[list[str]] = None, start_index: int = 0, end_index: int = -1) -> float:
        return self._get_weighted_sensor_average_single(self.get_total_gap_for_sensors(sensor_names, start_index, end_index))
    ...
    def _differentiate(self, data : list[float], n : int = 1, pad_front : bool = True) -> list[float]:
        """
        Differentiates the given data n times. If pad_front is True, the list is expanded by float("nan") n times
        """
        result = data
        for i in range(n):
            result = list(numpy.diff(result) / self._dt[i:])
        if pad_front:
    ...
    def _differentiate_all(self, data : dict[str, list[float]], n : int = 1, pad_front : bool = True) -> dict[str, list[float]]:
        """
        Calls SimulationGapData._differentiate for all entries in the given dictionary
        """
        result = {}
    ...
    def _integrate_all(self, data : dict[str, list[float]], n : int = 1, pad_front : bool = True) -> dict[str, list[float]]:
        """
        Calls SimulationGapData._integrate_all for all entries in the given dictionary
        """
        result = {}
        for key in data.keys():
    ...
    def _get(self,
            ...
            -> dict[str, list[float]]:
        """
        Returns all columns with the names "sensor_ + sensor_name + column_suffix" as dictionary.
        """
        if start_index < 0:
            start_index = 0
        if end_index == -1:
        ...
        for sensor_name in sensor_names:
            result[sensor_name] = ((self._merged["sensor_" + sensor_name + column_suffix])[start_index:end_index]).to_list()
        return result

    @staticmethod
    def _calculate_gap(d_extraction : dict[str, list[float]],
                       d_replay : dict[str, list[float]],
                       factor : float = 1) -> dict[str, list[float]]:
        gap = {}
        for sensor_name in d_extraction.keys():
            gap[sensor_name] = [pow((v_replay - v_extraction) / factor, 2)
                               for v_extraction, v_replay in zip(d_extraction[sensor_name], d_replay[sensor_name])]
        return gap

    @staticmethod
    def _get_weighted_sensor_average(dictionary: dict[str, list[float]]) -> list[float]:
    ...
    @staticmethod
    def _get_weighted_sensor_average_single(dictionary: dict[str, float]) -> float:
        return (numpy.average([dictionary[k] for k in dictionary.keys()],
                             weights=[WEIGHTS[k] for k in dictionary.keys()],
                             axis=0)
                .tolist()) # axis 0 = rows
    ...
    @staticmethod
    def _split_sensor_names(sensor_names : list[str]) -> tuple[list[str], list[str]]:
