...
class Simulator:
    ...
    def _set_experiment_parameters(self, experiment_data : ExperimentData) -> bool:
        ...
        if experiment_data.log_extraction:
            ...
            relative_path = ".." + "/Logs/ThesisFieldLogs/" + experiment_data.action_name + "/" + experiment_data.recording_date + "/" + (
                        str(experiment_data.log_index) + ".log")
    ...
    def run(self, experiment_datas : list[ExperimentData], batch_size : int, max_wait_for_ready : float = 10, max_run_duration : float = 20, gui : bool = True):
    ...
    def _run_batch(self, experiment_datas : list[ExperimentData], max_wait_for_ready : float = 10, max_run_duration : float = 20, gui : bool = True):
        ...
        try:
            for i in range(len(experiment_datas)):
                if not self._set_experiment_parameters(experiment_datas[i]):
                    ...
                    continue
                process_list.append(
                    subprocess.Popen(str(PATH_EXECUTABLE) + " " + str(PATH_LOG_EXTRACTION_SCENE) + ".ros2",
